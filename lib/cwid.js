...
async function platform_fetch_json(path) {
    try {
        let data_resp = await fsPromises.readFile(path)
        let data = data_resp.toString()
...
class CWID {
    ...
    constructor(hash_name,formats,multibase) {
        //
        (async () => {
            await this.init(hash_name,formats,multibase)
        })()
    }

    static use_blake3() {
        const {Blake3} = require('blake3-node-neon-no-package');
        const blake3_obj = new Blake3()
        blake3_hash = (str) => { return blake3_obj.hash(str) }
    }
    ...
    static async initialize() {
        ...
        if ( !ok ) throw new Error("fail to load multibase or formats or both")
        else {
            console.log("CWID assets initialized")
        }
    }
     ...
     */
    async init(hash_name,formats,multibase) {
        ...
        if ( formats === undefined ) {
            this.formats = g_formats
        }
        if ( multibase === undefined ) {
            this.multibase = g_multibase
        }
        this.hash_name = hash_name
        if ( (this.multibase === false) || (this.formats === false) ) {
    ...
    async _hash_of(text,base) {
        ...
        if ( base === 'base64url' ) {
            let hh = await _do_hash(text,base,this.hash_code)
            return hh.replace(/\=+/g,'')
    ...
    _base64_parts_to_hex_CWID(prefix,rest,from) {
        while ( prefix.length % 4 ) prefix += '='
        while ( rest.length % 4 ) rest += '='
        let preBuf = base64.base64ToBuffer(prefix,(from === 'base64'))
        let tailBuf = base64.base64ToBuffer(rest,(from === 'base64'))
        prefix = base_string.hex_fromBuffer(preBuf)
        rest = base_string.hex_fromBuffer(tailBuf)
    ...
    ipfs_cid_to_cwid(cid) {
        ...
        switch (code) {
            ...
            case 'u' : {
                let buf = base64.base64ToBuffer(bytes)
                let hexstr = base_string.hex_fromBuffer(buf)
    ...
    cwid_to_cid(cwid,base) {
        ...
        while ( p.length % 4 ) p += '='
        parts[0] = base64.base64ToBuffer(p,(base == 'base64'))
        p = parts[1]
        while ( p.length % 4 ) p += '='
        parts[1] = base64.base64ToBuffer(p,(base == 'base64'))
